...
- **Regression tests:** `tests/visual/workshop-pegboard.spec.ts` — describe **Workshop frame chrome (ADR-003)** asserts `toBeInViewport` on `.workshop-panel-nav` and the first `.pegboard-bg` at standard widths, screenshots `.portal-frame`, and a **375×500** short-viewport smoke. Run `npm run test:visual`.
