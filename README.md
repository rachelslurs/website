...
Design rationale for larger choices lives in **Architecture Decision Records** under [`docs/decisions/`](docs/decisions/) (for example [ADR-001: Workshop mobile pegboard layout contract](docs/decisions/001-workshop-mobile-pegboard-contract.md), [ADR-002: Visual regression CI favors speed over native build toolchain](docs/decisions/002-visual-regression-ci-speed-vs-native-builds.md), [ADR-003: Workshop frame chrome in the initial viewport](docs/decisions/003-workshop-frame-chrome-initial-viewport.md), [ADR-004: Workshop panel packing](docs/decisions/004-workshop-panel-packing.md), [ADR-009: Reading typography, prose boundaries, and theme vs CSS](docs/decisions/009-reading-typography-prose-and-theme.md), [ADR-010: Site-wide immersive pegboard shell](docs/decisions/010-site-wide-immersive-pegboard-shell.md), [ADR-011: Immersive shell DOM contract](docs/decisions/011-immersive-shell-dom-contract.md), and [ADR-012: Workshop URL → scene (pagination)](docs/decisions/012-workshop-url-scene-pagination.md), and [ADR-013: Phase 8 rollout wave 2+](docs/decisions/013-phase8-rollout-wave-2.md)).

**Workshop roadmap (phases + YAML todos):** [`.cursor/plans/responsive_pegcards_sizing_085cd05b.plan.md`](.cursor/plans/responsive_pegcards_sizing_085cd05b.plan.md) — pegboard responsive work, ADR-003/004 alignment, Phase 6 site shell / `site-pages` visuals; **Phase 7** reading typography (ADR-009): **7d–7e** shipped with Phase 8 pilot where noted in ADRs; **Phase 8** immersive shell (**ADR-010**, **ADR-011** pilot on workshop). **Maintenance:** when changing `RisoBoardShell` / workshop outer width, refresh **Docker** baselines for `workshop-pegboard` and `site-chrome` / `site-pages` if chrome shifts (see section above).
