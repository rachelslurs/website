...
When would you reach for `useRef` versus `useState` in a streaming context? It has an easy textbook answer and a much harder real one.

The textbook version is a one-liner about which hook triggers a re-render. I knew that part cold. What I hadn't internalized was what the difference actually _costs_ you when chunks are arriving every 50 milliseconds. So I did what I always do when something doesn't fully click: I built it until it did.

This post is that build, start to finish.
